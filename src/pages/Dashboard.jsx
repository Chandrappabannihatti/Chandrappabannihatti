...
function RecordsView({ user, students, achievements, onNavigate }) {
...
function LearnerOverview({ user, students, announcements, messages, remarks, achievements, onNavigate }) {
  const student = students.find((item) => item.usn === user.usn) || demoStudents[3]
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      api.getRemarks(),
      api.getAchievements(),
    ]).then(([studentResult, messageResult, announcementResult, remarkResult, achievementResult]) => {
      if (!mounted) return
  ...
  const renderContent = () => {
    ...
    if (user.role === 'student' || user.role === 'parent') {
      if (activeView === 'records') return <RecordsView user={user} students={students} achievements={achievements} onNavigate={navigateView} />
      ...
      return <LearnerOverview user={user} students={students} announcements={announcements} messages={messages} remarks={remarks} achievements={achievements} onNavigate={navigateView} />
    }
