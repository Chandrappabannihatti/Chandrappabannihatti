...
import { cloneDemoStudents, demoAnnouncements, demoMessages, demoRemarks, demoSections } from '../data/demo'
...
function SemesterAttendance({ user, semester, section = '', students, onNavigate }) {
  const scope = `${user.department} · Semester ${semester}${section ? ` · Section ${section}` : ''}`
  const trend = trendFor(students, semester)
...
function SemesterMarks({ user, semester, section = '', students, onNavigate }) {
  const scope = `${user.department} · Semester ${semester}${section ? ` · Section ${section}` : ''}`
...
function SemesterAchievements({ user, semester, section = '', students, onNavigate }) {
  const scope = `${user.department} · Semester ${semester}${section ? ` · Section ${section}` : ''}`
  return <div className="dashboard-content"><ScopeIntro eyebrow={scope} title="Achievements" description="Celebrate achievements recorded for students in this selected section." actions={<span className="scope-pill"><FiShield /> Scope locked</span>} /><div className="semester-context-banner"><FiShield /><span>Achievements below are generated from the academic signals of <strong>{scope}</strong> only.</span></div><section className="content-card table-card"><div className="table-card-head"><div><h2 className="card-title">Section achievements</h2><p className="card-description">Student highlights from the selected section.</p></div></div><div className="achievement-list section-achievement-list">{students.map((student) => <button className="section-achievement-row" key={student.id} type="button" onClick={() => onNavigate('student', student)}><Avatar initials={student.initials} tone="mint" /><span><strong>{student.name}</strong><small>{student.usn} · {achievementLabels(student).join(' · ')}</small></span><FiArrowRight /></button>)}{!students.length && <div className="empty-state"><FiAward /><span>No achievements for {scope} yet.</span></div>}</div></section></div>
}
...
function AddSemesterStudentPage({ user, semester, section = '', students, onCancel, onSave }) {
  const [form, setForm] = useState({ usn: '', name: '', section: section || 'A', gender: 'Female', email: '', phone: '', parentName: '', parentPhone: '', attendance: 75, cgpa: 7, ia1: 30, ia2: 30, assignmentMarks: 15, previousSgpa: 7, backlogs: 0 })
...
export function TeacherStudentProfile() {
  ...
  const [student, setStudent] = useState(() => cloneDemoStudents().find((item) => item.department === user.department && item.usn.toUpperCase() === usn) || null)
  const [remarks, setRemarks] = useState(() => demoRemarks)
  const [loading, setLoading] = useState(!DEMO_MODE)

  useEffect(() => {
    if (DEMO_MODE) {
      setStudent(cloneDemoStudents().find((item) => item.department === user.department && item.usn.toUpperCase() === usn) || null)
      setLoading(false)
      return undefined
    }
    let mounted = true
    Promise.allSettled([
      api.getStudents({ department: user.department, search: usn, limit: 100 }),
      api.getRemarks({ department: user.department }),
    ]).then(([studentResult, remarkResult]) => {
      if (!mounted) return
      const rows = studentResult.status === 'fulfilled' ? studentResult.value?.data || [] : []
      setStudent(rows.find((item) => item.department === user.department && item.usn?.toUpperCase() === usn) || null)
  ...
  const content = loading ? <div className="dashboard-content"><div className="content-card empty-state"><FiUsers /><span>Loading the student profile…</span></div></div> : <StudentProfilePage user={user} semester={semester} student={student} remarks={remarks.filter((remark) => !student || Number(remark.studentId) === Number(student.id))} onNavigate={onNavigate} />
...
export default function TeacherSemester() {
  ...
  useEffect(() => {
    ...
    ]).then(([sectionResult, studentResult, announcementResult, messageResult, remarkResult]) => {
      if (!mounted) return
  ...
  const navigateModule = (module, id) => {
    if (module === 'student') {
      ...
      if (record?.usn) return navigate(`/teacher/student/${encodeURIComponent(record.usn)}`)
  ...
  else if (sectionModule === 'attendance') content = <SemesterAttendance user={user} semester={semester} section={section} students={sectionStudents} onNavigate={navigateModule} />
  else if (sectionModule === 'marks') content = <SemesterMarks user={user} semester={semester} section={section} students={sectionStudents} onNavigate={navigateModule} />
  ...
  else if (sectionModule === 'achievements') content = <SemesterAchievements user={user} semester={semester} section={section} students={sectionStudents} onNavigate={navigateModule} />
