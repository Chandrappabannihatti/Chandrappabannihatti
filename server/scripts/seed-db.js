...
for (const item of demoStudents) {
  ...
  const [studentResult] = await pool.query(`INSERT INTO students (usn, name, department_code, semester, section, section_id, gender, date_of_birth, blood_group, address, email, phone, parent_name, father_name, mother_name, parent_phone, parent_email, certifications, skills, attendance_percentage, average_internal_marks, average_assignment_score, previous_gpa, current_gpa, participation_score, result, risk, password_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id), name=VALUES(name), department_code=VALUES(department_code), semester=VALUES(semester), section=VALUES(section), section_id=VALUES(section_id), date_of_birth=VALUES(date_of_birth), blood_group=VALUES(blood_group), address=VALUES(address), father_name=VALUES(father_name), mother_name=VALUES(mother_name), parent_email=VALUES(parent_email), certifications=VALUES(certifications), skills=VALUES(skills), attendance_percentage=VALUES(attendance_percentage), average_internal_marks=VALUES(average_internal_marks), average_assignment_score=VALUES(average_assignment_score), previous_gpa=VALUES(previous_gpa), current_gpa=VALUES(current_gpa), participation_score=VALUES(participation_score), result=VALUES(result), risk=VALUES(risk), password_hash=VALUES(password_hash)`,  [item.usn, item.name, item.department, item.semester, item.section, sectionRow?.section_id || null, item.gender, item.dateOfBirth || null, item.bloodGroup || null, item.address || null, item.email, item.phone, item.parentName, item.fatherName || item.parentName, item.motherName || null, item.parentPhone, item.parentEmail || null, JSON.stringify(item.certifications || []), JSON.stringify(item.skills || []), item.attendancePercentage, item.averageInternalMarks, item.averageAssignmentScore, item.previousGpa, item.currentGpa, item.participationScore, item.result, item.risk, hashes.student])
  const studentId = studentResult.insertId
  await pool.query('INSERT INTO users (role, display_name, email, password_hash) VALUES ("student", ?, ?, ?) ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id), password_hash=VALUES(password_hash)', [item.name, item.email, hashes.student])
  ...
  await pool.query('INSERT INTO attendance (student_id, subject_code, attendance_date, status, attendance_percentage, marked_by) VALUES (?, "ALL", CURDATE(), "Present", ?, ?) ON DUPLICATE KEY UPDATE attendance_percentage=VALUES(attendance_percentage), marked_by=VALUES(marked_by)', [studentId, item.attendancePercentage, teacherId])
  await pool.query('INSERT INTO predictions (student_id, attendance_percentage, average_internal_marks, average_assignment_score, previous_gpa, participation_score, result, risk) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ', [studentId, item.attendancePercentage, item.averageInternalMarks, item.averageAssignmentScore, item.previousGpa, item.participationScore, item.result, item.risk])
}
...
await pool.end()
console.log('CAMPS demo data seeded with Average Academic Performance values.')
